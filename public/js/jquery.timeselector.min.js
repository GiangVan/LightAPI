(function($){var Timeselector=function(){this._$div=null;this._inst=null;this._showing=!1;this.uuid=new Date().getTime();this.initialized=!1};Timeselector.prototype={constructor:Timeselector,_generateHtml:function(){return'<div id="timeselector-div" class="timeselector" style="width:143px;height:100px;">'+'<div class="timeselector-item timeselector-hour" data-type="hour">'+'<a class="timeselector-button timeselector-up" style="width:64px;height:35px;">+</a>'+'<span class="timeselector-value"></span>'+'<a class="timeselector-button timeselector-down" style="width:64px;height:35px;">-</a>'+'</div>'+'<div class="timeselector-separator">:</div>'+'<div class="timeselector-item timeselector-minute" data-type="minute">'+'<a class="timeselector-button timeselector-up" style="width:64px;height:35px;">+</a>'+'<span class="timeselector-value"></span>'+'<a class="timeselector-button timeselector-down" style="width:64px;height:35px;">-</a>'+'</div>'+'</div>'},_setTime:function(inst,step){var date=inst.date,min=inst.options.min,max=inst.options.max,value,limit;if(step){value=+date+(1000*60*step);min=this._match(min)&&this._parseTime(min);max=this._match(max)&&this._parseTime(max)
...
this._lastInput=input;this._setTimeFromField(inst);this._update(inst);$input.parents().each(function(){isFixed=$(this).css('position')==='fixed';return!isFixed});pos=$input.offset();pos.top+=input.offsetHeight;pos.left-=isFixed?$(document).scrollLeft():0;pos.top-=isFixed?$(document).scrollTop():0;inst.div.css({position:(isFixed?'fixed':'absolute'),display:'none',top:pos.top,left:pos.left,zIndex:this._zIndex(input)+1});inst.div.fadeIn('fast');this._showing=!0;this._inst=inst},_hide:function(){var inst=this._inst;if(!inst||!this._showing){return}
